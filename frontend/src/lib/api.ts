import { Job, Lead, LeadsPaginationResponse, SettingsResponse, StatsSummary, AuditReport } from "@/types";

const API_BASE = process.env.NEXT_PUBLIC_API_BASE || "http://127.0.0.1:8000/api";

export async function startScrapeJob(niche: string, state: string): Promise<Job> {
  const res = await fetch(`${API_BASE}/scrapes/start`, {
    method: "POST",
...
export async function getJobs(limit: number = 50, offset: number = 0): Promise<{ jobs: Job[]; total: number }> {
  const res = await fetch(`${API_BASE}/scrapes/?limit=${limit}&offset=${offset}`, {
    cache: "no-store",
  });
...
export async function getJob(jobId: string): Promise<Job> {
  const res = await fetch(`${API_BASE}/scrapes/${jobId}`, { cache: "no-store" });
  if (!res.ok) throw new Error("Failed to fetch job details");
  return res.json();
}

export async function deleteJob(jobId: string): Promise<void> {
  const res = await fetch(`${API_BASE}/scrapes/${jobId}`, { method: "DELETE" });
...
export async function getLeads(params: GetLeadsParams = {}): Promise<LeadsPaginationResponse> {
  ...
  const res = await fetch(`${API_BASE}/leads/?${query.toString()}`, { cache: "no-store" });
...
export async function getWebsiteAudit(leadId: string): Promise<AuditReport> {
  const res = await fetch(`${API_BASE}/audit/${leadId}`, { cache: "no-store" });
  if (!res.ok) {
    const errorData = await res.json().catch(() => ({}));
...
export function getAuditPdfUrl(leadId: string): string {
  return `${API_BASE}/audit/${leadId}/pdf`;
}
